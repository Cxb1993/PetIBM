...
PETSC_PETSCVARIABLES=`find ${PETSC_DIR}/${PETSC_ARCH} -name petscvariables`
AC_SUBST(PETSC_PETSCVARIABLES)
...
AC_MSG_NOTICE([using PETSC_PETSCVARIABLES: ${PETSC_PETSCVARIABLES}])
AC_MSG_NOTICE([using PETSC_VARIABLES: ${PETSC_VARIABLES}])

PETSC_CC_INCLUDES=`grep "PETSC_CC_INCLUDES =" ${PETSC_PETSCVARIABLES} | sed -e 's/.*=//' -e 's/^[ \t]*//'`
PETSC_EXTERNAL_LIB_BASIC=`grep "PETSC_EXTERNAL_LIB_BASIC =" ${PETSC_PETSCVARIABLES} | sed -e 's/.*=//' -e 's/^[ \t]*//'`
PETSC_WITH_EXTERNAL_LIB=`grep "PETSC_WITH_EXTERNAL_LIB =" ${PETSC_PETSCVARIABLES} | sed -e 's/.*=//' -e 's/^[ \t]*//'`
